...
from src.database.func import get_all_cars, get_spec_cars_number, get_all_spec_models

class All_cars(StatesGroup):
    init_model = State()
...
async def get_all_model_car(message: Message, state: FSMContext):
    await state.set_state(All_cars.init_model)
    await message.reply('Выберите интересующую модель', reply_markup=await reply.check_number())

@router_cars_model.message(All_cars.init_model)
