...
chevr_url = ("https://abw.by/cars/brand_chevrolet/model_equinox/"
             "generation_iii/price_{min_price}:{max_price}/engine_benzin/"
             "transmission_at#classified-listing-adverts")
volvo_url = ("https://abw.by/cars/brand_volvo/model_xc60/generation_i/price_{min_price}"
...
hyundai_url = ('https://abw.by/cars/brand_hyundai/model_santa-fe/'
...
async def parser_abw_by(
    ...
    async with session.get(
        ...
        data_site = soup.find_all(
            ["div", "span"],
            {"class": ["top__left", "price-usd", 'price-byn' ,"bottom__additional"]},
        )
        result = {}
        for i in data_site:
            if i.attrs['class'] == ['top__left']:
                data = i.find('a', {'class': 'top__title'}).get('href').split('/')
                model = data[4]
                if model in model_dict:
                    ...
                    result[2] = link_
                    odometer = int(''.join(i.find('ul', {'class': 'top__params'}).contents[1].text.split()[:-1]))
                    result[3] = odometer
            elif i.find("span", {"class": "bottom__city"}):
                date_pub = i.contents[1].text
                result[4] = date_pub
            ...
            if len(result) == 6:
                async with get_db_session() as session:
                    data: Result[tuple[Cars]] = await session.execute(select(Cars).where(Cars.link == result[2]))
                    ...
                    if date_result is not None:
                        if date_result.price_blr != result[5]:
                            await session.execute(update(Cars).
                                                  ...
                                                  values(price_blr=result[5], price_usd=result[6]))
